import styles from './ManuallyForm.module.css'
...
export interface ManuallyFormValues {
    MovieTitle: string
    ImgUrl: string,
    Descreption: string,
    Month: string,
...
export default function ManuallyForm({ movie }: MovieFormProps) {
    const queryClient = useQueryClient();
    const navigate = useNavigate();
    const [showError, setShowError] = useState(false) // show or hide error dialog 

    const { control, handleSubmit } = useForm<ManuallyFormValues>({
        defaultValues: {
    ...
    // remove erro pop-up after 3 seconds
    useEffect(() => {
        if (showError) {
            setTimeout(() => {
                setShowError(false)
    ...
    const onSubmit: SubmitHandler<ManuallyFormValues> = (data) => {
        if (movie) {
